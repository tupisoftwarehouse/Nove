...
class TimeUnitConverterScreen extends StatefulWidget {
  final TimeUnitConverterViewModel timeUnitConverterViewModel;
  final Router router;

  const TimeUnitConverterScreen({
    super.key,
  ...
  @override
  State<TimeUnitConverterScreen> createState() {
    return _TimeUnitConverterScreenState();
  }
}

class _TimeUnitConverterScreenState extends State<TimeUnitConverterScreen> {
  late final ThemeViewModel _themeViewModel;
  late final DeviceLanguageStringsViewModel _deviceLanguageStringsViewModel;
  late final ValueNotifier<List<TimeUnitInput>> _listOfTimeUnitInputs;
  ...
  void initState() {
    ...
    _listOfTimeUnitInputs = ValueNotifier([]);
    ...
    ListOfTimeUnitInputDataLoadingTransactionScript.loadListOfTimeUnitInputData(
      _listOfTimeUnitInputs,
      widget.timeUnitConverterViewModel,
      _deviceLanguageStringsViewModel,
    );

    widget.timeUnitConverterViewModel.listOfTimeUnits.addListener(() {
      if (isListOfMeasureUnitsResized(
        widget.timeUnitConverterViewModel.listOfTimeUnits.value.length,
        _listOfTimeUnitInputs.value.length,
      )) {
        ListOfTimeUnitInputDataLoadingTransactionScript.loadListOfTimeUnitInputData(
          _listOfTimeUnitInputs,
          widget.timeUnitConverterViewModel,
    ...
    widget.timeUnitConverterViewModel.baseTimeUnitIndex.addListener(() {
      ListOfTimeUnitInputDataUpdatingTransactionScript.updateListOfTimeUnitInputData(
        _listOfTimeUnitInputs,
        widget.timeUnitConverterViewModel,
        _deviceLanguageStringsViewModel,
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final calculateButtonContainerRenderBox =
          _bottomPanelKey.currentContext!.findRenderObject() as RenderBox;

      _bottomPanelHeight.value = calculateButtonContainerRenderBox.size.height;
    });
  }
            ...
            Scaffold(
              ...
              actionButtons: [
                ValueListenableBuilder(
                  valueListenable:
                      widget.timeUnitConverterViewModel.listOfTimeUnits,
                  builder: (_, listOfTimeUnits, _) {
                    return AnimatedScale(
                      scale: isConverterUnchanged(listOfTimeUnits) ? 0 : 1,
                      ...
                      child: ActionButton(
                        ...
                        onTap: () {
                          widget.timeUnitConverterViewModel.reset();

                          _listOfTimeUnitInputs.value.first.controller.text =
                              ZEROED_VALUE;

                          _listOfTimeUnitInputs.value = [
                            _listOfTimeUnitInputs.value.first,
                          ];
                      ...
                      children: [
                        ...
                        Padding(
                          ...
                          child: IconButton(
                            ...
                            onTap:
                                widget.timeUnitConverterViewModel.addTimeUnit,
                          ),
                        ),
                      ],
                    ),
                  ...
                  return isSnackbarShown
                      ? Snackbar(
                          message: deviceLanguageStrings
                              .RESTORE_DELETED_LIST_OF_TIME_UNITS_MESSAGE,
                          description: deviceLanguageStrings
                              .RESTORE_DELETED_LIST_OF_TIME_UNITS_DESCRIPTION,
                          ...
                          onTap: () {
                            widget.timeUnitConverterViewModel.restore();
                          ...
                          onTimeout: () {
                            widget.timeUnitConverterViewModel.dismissRecovery();
